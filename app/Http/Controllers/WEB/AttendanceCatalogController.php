...
class AttendanceCatalogController extends Controller
{
    ...
    private function validateWorkShift(Request $request, ?WorkShift $workShift = null): array
    {
        ...
        $handoverBreakMinutes = (int) ($validated['handover_break_minutes'] ?? 0);
        $netMinutes = $workMinutes - $breakMinutes - $handoverBreakMinutes;
        if ($netMinutes <= 0) {
            throw ValidationException::withMessages([
                'handover_break_minutes' => 'Tong thoi gian nghi khong duoc bang hoac lon hon tong thoi gian ca lam.',
            ]);
        }

        if ((int) $validated['standard_minutes'] !== $netMinutes) {
            throw ValidationException::withMessages([
                'standard_minutes' => 'Phut chuan phai khop voi thoi luong ca sau khi tru nghi (' . $netMinutes . ' phut).',
            ]);
        }
