      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          <select v-model="requestForm.request_type" class="w-full rounded-lg border border-gray-300 px-3 py-2">
            ...
            <option v-for="type in request_types" :key="type.value" :value="type.value">{{ type.label }}</option>
          </select>
        </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-sm">
      <DataTable :columns="columns" :data="records" empty-message="Chưa có dữ liệu chấm công.">
        ...
        <template #cell-attendance_status="{ item }">
          <span :class="statusClass(item.attendance_status)" class="rounded-full px-3 py-1 text-xs font-semibold">
            {{ formatAttendanceStatus(item.attendance_status) }}
          </span>
        </template>
...
const summaryCards = computed(() => [
  ...
  { label: 'Tổng giờ làm', value: formatHours(props.summary.total_worked_minutes ?? 0) },
])
